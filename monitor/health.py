# -*- coding: utf-8 -*-
"""数据健康检查：定时核检 monitor.db，发现断档 / 脏值即告警，并输出每日健康心跳。

目的：守护长期累积的数据质量。数据静默变质（断档、脏值）若不在早期发现，
会在后续建模/回测时造成难以定位的错误。

两层防线：
1. 写入时校验（db.py）：NaN/inf、非正 OI 等脏值在入库前拦截
2. 定时核检（本模块）：断档 / 脏值 / 币种覆盖

告警策略：仅在「好→坏」状态切换时告警（按稳定 key 去重），避免断档期间重复刷屏；
恢复后发送「已恢复」。健康心跳每日一条，用于确认采集进程持续存活。
"""
